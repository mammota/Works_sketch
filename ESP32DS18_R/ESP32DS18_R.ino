...
// Пин, к которому подключен модуль DS18B20
const int oneWireBus = 22;

// Создаем экземпляр объекта OneWire для общения с датчиками
...
void loop() {
  ...
  // Проверяем, успешно ли удалось считать температуру
  if (temperature) {
    // Если считывание прошло успешно, выводим температуру на монитор последовательного порта
    Serial.print("Температура: ");
    Serial.print(temperature);
    Serial.println("°C");
  } else {
    // Если считывание не удалось, выводим сообщение об ошибке
    Serial.println("Ошибка при считывании температуры!");
  }

  // Задержка в 1 секунду перед следующим измерением
  delay(1000);
}

// Функция для считывания температуры с датчика DS18B20
